...
$joinQuery = "from (SELECT DISTINCT (a.wo_no),EXTRACT(WEEK FROM a.ex_fty_date) as week, 			
					...
					JOIN (select sum(gdp_goods) totalout,wo_no,color,DATE(ex_fty_date) as ex_fty_date
							   from 
							   (SELECT A.qrcode_key,workcenter_id,gdp_datetime,b.wo_no,b.color,b.ex_fty_date,to_timestamp( FLOOR ( ( EXTRACT ( 'epoch' FROM gdp_datetime ) / 300 ) ) * 300 ) AT TIME ZONE'UTC' + INTERVAL '5 minute' AS interval_alias,
							COALESCE ( smv_minute :: NUMERIC, '0' :: NUMERIC ) AS smv,gdp_goods 
								 FROM qrcode_gdp A 
...
$columns = array(
	...
	array( "db" => "totalout", 	"dt" => 14, "field" => "totalout" ),
	array( "db" => "wo_master_dtl_proc_qty_rec", 	"dt" => 15, "field" => "wo_master_dtl_proc_qty_rec" ),
	array( "db" => "sewing_balance", 	"dt" => 16, "field" => "sewing_balance" ),
	array( "db" => "totaloutsew", 	"dt" => 17, "field" => "totaloutsew" ),
	array( "db" => "lastdate", 	"dt" => 18, "field" => "lastdate" ),
	array( "db" => "total_wip", 	"dt" => 19, "field" => "total_wip",
