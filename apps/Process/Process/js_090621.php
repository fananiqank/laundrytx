<script>
	...
	function cekuser(lot,user,usercode,master_type_process_id){
		...
		if (lot == ''){
			...
			//alert(lotres);	
			$('#tampilcmt').load("apps/Process/Process/tampilcmt.php?lot="+lot+"&usercode="+usercode+"&typelot="+lotres);
				...
				swall("Fill Time Machine","You Must Fill Time Machine, Can not 0","error",2000);
					...
					.then((willDelete) => {
					  if (willDelete) {
						...
						// 	data: data,
						// 	success: function() {
								dealcorrect(lot,user,role,typelot,master_type_process,master_process_id,no_id,roledtlid,parent,usemachine,role_wo_id,typeqc,qtylast,type_sequence,usercode);
						// 	}
						// });
	...
	function dealcorrect(lot,user,role,typelot,master_type_process,master_process_id,no_id,roledtlid,parent,usemachine,role_wo_id,typeqc,qtylast,type_sequence,usercode){
		if (master_type_process == 2) {
			...
			if (master_process_id == '42'){
				...
				$('#tampilcontent').load("apps/Process/Dry/content_process.php?lot="+lot+"&user="+user+"&role="+role+"&typelot="+typelot+"&q="+$('#qty_process').val()+"&parent="+parent+"&mpd="+master_process_id+"&rwid="+role_wo_id+"&usercode="+usercode);
			...
			$('#tampilcontent').load("apps/Process/Wet/content_process.php?lot="+lot+"&user="+user+"&role="+role+"&typelot="+typelot+"&q="+$('#qty_process').val()+"&parent="+parent+"&mpd="+master_process_id+"&rwid="+role_wo_id+"&usercode="+usercode);
			loopnotif(lot,user,role,typelot,$('#qty_process').val(),master_type_process,parent,master_process_id,role_wo_id,usercode);
		}  
	...
	function loopnotif(lot,user,role,typelot,qty,master_type_process,parent,master_process_id,role_wo_id,usercode){
		if(master_type_process == 4){
			 myLoop = setInterval(function(){ $('#tampilcontent').load("apps/Process/Dry/content_process.php?lot="+lot+"&user="+user+"&role="+role+"&typelot="+typelot+"&q="+qty+"&mtype="+master_type_process+"&parent="+parent+"&mpd="+master_process_id+"&rwid="+role_wo_id+"&usercode="+usercode);  }, 5000);

		} else {
			 myLoop = setInterval(function(){ $('#tampilcontent').load("apps/Process/Wet/content_process.php?lot="+lot+"&user="+user+"&role="+role+"&typelot="+typelot+"&q="+qty+"&mtype="+master_type_process+"&parent="+parent+"&mpd="+master_process_id+"&rwid="+role_wo_id+"&usercode="+usercode);  }, 5000);
		}
	}
			...
			$('#process-status').val(a);
					...
					$.ajax({
						...
						success: function(lot) {
							...
							$('#tampilcontent').load("apps/Process/Process/content_isi.php?usercode="+$("#usercode").val()+"&stat=process");
							$("#lot_no").focus();
						}
					});
		}
