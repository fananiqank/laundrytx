...
//process start
	...
	if ($_POST['usemachine'] == 1) {
		...
		$idseqmachine = $con->idseq("laundry_process_machine_progress","process_machine_sequence","lot_no","lot_no");
...
//process End
	...
	foreach($con->select("(select process_qty_good from laundry_process a join (select max(process_id) processid from laundry_process where lot_no = '$_POST[lot_no_process]') b on a.process_id=b.processid) a","*") as $qtylot){}

	$qtygood = $qtylot['process_qty_good']-$_POST['qtystd'];
	...
	$idprocess = $con->idurut("laundry_process","process_id");
		...
		$dataprocess = array(
						 ...
						 'process_status' => 1,
						 'machine_id' => $_POST['machineid'],
						 'process_qty_good' => $qtygood,
						 'process_qty_reject' => 0,
					 	 'process_qty_repair' => 0,
						 'process_qty_total' => $qtylot['process_qty_good'],
