	...
	swal({
		  ...
		  dangerMode: true,
		  	...
		  	if (willDelete) {
				...
				$.ajax({
					type: 'POST',
					url:  "apps/Process/Process/simpan.php?mtpid="+master_type_process_id,
					data: data,
					success: function(lot) {
							// if (lot != ''){
							// 	explotout=lot.split("_");
...
function cstd()
{
  if (document.getElementById('cekstd').checked) 
  {
      ...
      document.getElementById("qtystd").value = '';
      document.getElementById("qtystd").focus = true;
  } else {
  	
      document.getElementById("qtystd").readOnly = true; 
  	  document.getElementById("qtystd").value = 0;
      document.getElementById("qtystd").focus = false;
  }
}
