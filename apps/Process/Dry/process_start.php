...
<div class="form-group" id="button-process">
  <?php 
    if ($cmt['master_process_usemachine'] != 0) {
       include "button-process.php";
    } else if ($_GET['usemachine'] != 0){
       include "button-process.php";
    }else {
       include "button-process-nonmachine.php";
    }
  ?>
...
<div class="form-group">
  ...
  <div class="col-sm-6 col-md-6 col-lg-6" style="font-size: 14px;">
    ...
    <div class="row">
      ...
      ?>
      <a href="javascript:void(0)" data-toggle='modal' data-target='#funModal' id='mod' class="btn btn-primary" style="padding: 3%;background-color: #FF4500" onclick="model('<?=$mach[machine_id]?>','<?=$lot?>','<?=$_GET[user]?>',3,0,'<?=$qty?>','<?=$masterprocid?>','<?=$_GET[typelot]?>')" <?php echo $disableout; ?>><b style="font-size:20px;">OUT</b></a>
    </div>
  </div>
</div>    