...
$joinQuery = "from (select a.rec_id,a.rec_no,a.rec_status,a.wo_no,a.garment_colors,b.color_wash,
					CASE WHEN COALESCE(a.rec_qty_good_upd,0) != 0 THEN a.rec_qty_good_upd 
		                         ELSE a.rec_qty END as qty,a.rec_createdby,a.user_code,c.username,d.shift_name,rec_break_status,CONCAT(a.rec_no,'_',a.rec_type) as rec_no_type,b.ex_fty_date, to_char(b.ex_fty_date, 'DD-MM-YYYY') as ex_fty_date_show
					...
					GROUP BY a.rec_no,a.rec_status,a.wo_no,a.garment_colors,b.ex_fty_date,a.rec_id,c.username,d.shift_name,b.color_wash) as asi";
...
$columns = array(
	...
	array(
		...
		'formatter' => function ($d, $row) {
			...
			$isijum = $detail."<a href='lib/pdf-qrcode.php?c=".base64_encode($recno)."' class='label label-warning' style='cursor:pointer' target='_blank'><i class='fa fa-print' aria-hidden='true' style='font-size:14px;'></i></a>";
