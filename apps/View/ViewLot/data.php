...
$primaryKey = 'shift_id';
// $joinQuery = "from (select a.lot_id,a.lot_no,a.wo_no,a.garment_colors,a.lot_qty,a.lot_kg,a.lot_status,c.color_wash,
// 					CASE 
// 						WHEN COALESCE(a.lot_qty_good_upd,0) != 0
// 						THEN COALESCE(a.lot_qty_good_upd,0)
// 						ELSE a.lot_qty
// 					END as qty,
// 					concat(a.lot_no,'_',a.lot_status,'_',e.process_id,'_',a.lot_type) as act,a.lot_shade,
...
$joinQuery = "from (select a.lot_id,a.lot_no,a.wo_no,a.garment_colors,a.lot_qty,a.lot_kg,a.lot_status,c.color_wash,
					CASE 
						WHEN COALESCE(f.process_qty_good,0) != 0
						THEN COALESCE(f.process_qty_good,0) ELSE a.lot_qty
					END as qty,
					concat(a.lot_no,'_',a.lot_status,'_',e.process_id,'_',a.lot_type) as act,a.lot_shade,
					to_char(c.ex_fty_date, 'DD-MM-YYYY') as ex_fty_date_show,
					DATE(ex_fty_date) as ex_fty_date,
					...
					LEFT JOIN (select max(process_id) as process_id,lot_no from laundry_process where process_type != 5 GROUP BY lot_no) as e on 
					a.lot_no=e.lot_no
					LEFT JOIN (select max(process_id) as process_id,lot_no,process_qty_good from laundry_process where process_type != 5 GROUP BY lot_no,process_qty_good) f on e.process_id=f.process_id
					GROUP BY a.lot_id,a.lot_no,a.wo_no,a.garment_colors,a.lot_qty,a.lot_kg,a.lot_status,c.color_wash,a.lot_shade,
					c.ex_fty_date,a.user_code,e.process_id,f.process_qty_good
				) as an";  
...
$columns = array(
	...
	array( "db" => "act", 	"dt" => 12, "field" => "act" ,
			'formatter' => function( $d, $row ) {
				...
				if($expact[3] != 'M' && $expact[3] != 'W' && $expact[3] != 'S' && $expact[3] != 'T'){
