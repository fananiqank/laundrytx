...
$joinQuery = "from (select a.lot_id,a.lot_no,a.wo_no,a.garment_colors,a.lot_qty,a.lot_kg,a.lot_status,c.color_wash,
					CASE 
						...
						ELSE a.lot_qty
					END as qty,
					concat(a.lot_no,'_',a.lot_status) as act,
					coalesce(b.shift_name, '-') AS shift_name,a.lot_shade,
					...
					LEFT JOIN laundry_master_shift b on a.shift_id=b.shift_id) as an";  
...
$columns = array(
	...
	// array( "db" => "shift_name",     "dt" => 8, "field" => "shift_name" ),
	array( "db" => "lot_id", 	"dt" => 9, "field" => "lot_id" ,
	...
	array( "db" => "lot_status", 	"dt" => 10, "field" => "lot_status" ,
			'formatter' => function( $d, $row ) {
					if ($d == '1'){
