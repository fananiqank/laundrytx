...
//jumlah hasil scan
foreach ($selcountcart = $con->select("laundry_scan_qrcode", "count(scan_id) as jmlscan", "scan_status = 0 and scan_createdby = '" . $_SESSION['ID_LOGIN'] . "' and scan_type = '3' and lot_no = '" . $nolot . "'") as $cart) {
} 
...
<input type="hidden" name="cutqtywo" id="cutqtywo" value="<?php echo $cut[cutting_qty]; ?>">
<input type="hidden" name="totalqtyrec" id="totalqtyrec" value="<?php echo $totalrec; ?>">
<input type="hidden" name="totalqtyscan" id="totalqtyscan" value="<?php echo $cart[jmlscan]; ?>">