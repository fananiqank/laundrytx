<div class="form-group"  style="font-size: 14px;">
	<div class="col-md-1" align="left">
  	...
  	<?php 
  	if (($_GET['taskprocess'] == 1 && $role1['master_type_process_id'] == 3)) {
	...
	<?php } else if ($role1['dtl_process'] == '1'){ ?> <!--Process Start-->
	...
	<?php } else if ($role1['dtl_process'] == '2'){ ?> <!--Process Out-->
			<a href="javascript:void(0)" class="btn btn-success" style="padding: 3%;background-color: #800000"  <?=$disend?> onclick="process_end(3)"><b style="font-size:20px;">END</b></a>
	<?php 
		} 

	} else if ($_GET['taskprocess'] == 2 && $role1['master_type_process_id'] == 6) {
		if($role1['dtl_process'] == ''){ 
  	?> <!--Process IN-->
	  		<a href="javascript:void(0)" class="btn btn-warning" style="padding: 3%;" onclick="process_in(1,'<?=strtoupper($_GET[lot])?>','<?=$cmt[master_process_usemachine]?>','<?=$role1[master_process_id]?>','<?=$cmt[role_dtl_wo_time]?>','<?=$cmt[qty]?>')" <?=$disin?>><b style="font-size: 20px;">&emsp;IN&emsp;</b></a>
	<?php } else if ($role1['dtl_process'] == '1'){ ?> <!--Process Start-->
	   		<a href="javascript:void(0)" class="btn btn-success" style="padding: 3%;background-color: #800000"  <?=$disend?> onclick="process_end(3)"><b style="font-size:20px;">END</b></a>
	<?php } else if ($role1['dtl_process'] == '4'){ 
			echo "<a href='javascript:void(0)' class='btn btn-primary' style='padding: 3%;'><b style='font-size:14px;'>Sequence Done</b></a>";
		} 
