		...
		foreach ($con->select("(
						...
						FROM
							...
							LEFT JOIN ( SELECT role_dtl_wo_id, process_qty_good,lot_no,
										role_wo_id FROM (select wo_master_dtl_proc_id,process_qty_total,master_type_process_id,master_process_id,process_type,max(process_createdate) as process_createdate,role_wo_id,role_dtl_wo_id,COALESCE(process_qty_good,0) as process_qty_good,COALESCE(process_qty_reject,0) as process_qty_reject,COALESCE(process_qty_repair,0) as process_qty_repair,COALESCE(process_qty_std,0) as process_qty_std,wo_no,garment_colors,role_wo_name_seq,role_wo_master_id,lot_no from laundry_process GROUP BY lot_no, wo_master_dtl_proc_id,process_qty_total,master_type_process_id,master_process_id,process_type,role_wo_id,role_dtl_wo_id,COALESCE(process_qty_good,0),COALESCE(process_qty_reject,0),COALESCE(process_qty_repair,0),COALESCE(process_qty_std,0),wo_no,garment_colors,role_wo_name_seq,role_wo_master_id,lot_no) as jo WHERE role_wo_master_id = '".$rolewomasterid."' AND lot_no = '". strtoupper($_GET['lot']) ."' AND process_type = 4 ORDER BY process_type DESC ) AS e ON e.role_dtl_wo_id = b.role_dtl_wo_id 
						WHERE
							role_wo_master_id = '".$rolewomasterid."' AND 
...
(SELECT trim(wo_no) as wo_no, trim(color) as garment_colors,
  ...
  FROM qrcode_ticketing_detail a join qrcode_ticketing_master b ON a.ticketid=b.ticketid WHERE wo_no = '$cmt[wo_no]' and color = '$cmt[garment_colors]' GROUP BY wo_no,color ) as asi
	...
	(SELECT trim(wo_no) as wo_no, trim(garment_colors) as garment_colors,SUM(wo_master_dtl_desp_qty) as qty	FROM laundry_wo_master_dtl_despatch A JOIN laundry_wo_master_dtl_proc E ON A.wo_master_dtl_proc_id = E.wo_master_dtl_proc_id 
		WHERE	wo_master_dtl_desp_status = 1 and wo_no = '$cmt[wo_no]' and garment_colors = '$cmt[garment_colors]' GROUP BY wo_no, garment_colors) AS aso
	ON asi.wo_no=aso.wo_no and asi.garment_colors=aso.garment_colors) as asu","*") as $cekqrout){}
	?>
			...
			<div class="form-group"  style="font-size: 12px;">
				...
				<label class="col-md-2 control-label" for="profileLastName"><b>Qty (QR Out)</b></label>
					<div class="col-md-2">
						<?php echo $cekqrout['qroutput']; ?>
					</div>
					<label class="col-md-1 control-label" for="profileLastName"><b>Blnc</b></label>
					<div class="col-md-1">
						<?php echo $cekqrout['balance_desp']; ?>
					</div>
			</div>
			<div class="form-group"  style="font-size: 12px;">
