<script>
	...
	function lcode () {
    	if(event.keyCode == 13){// => Karakter enter dikenali sebagai angka 13
	        var lot = $("#lot_no").val();
	        var user = $("#userid").val();
	        var usercode = encodeURI($("#usercode").val());
	...
	function cekuser(lot,user,usercode,master_type_process_id){
		...
		//$('#tampilcmt').load("apps/Process/Process/tampilcmt.php?lot="+lot+"&usercode="+usercode+"&typelot="+lotres+"&taskprocess=1");
			$('#tampilcmt').load("apps/Transaction/QC_scan/tampilcmt2.php?lot="+lot+"&user="+user+"&typelot="+lotres+"&taskprocess=1");
		}
	}

	function enterlot(a){
		$('#tampilcontent').load("apps/Transaction/QC_scan/content_create.php");
	}

	function reclot(a){
		$('#formku').load("apps/Transaction/QC_scan/content_isi.php?id="+a);
		$('#button_rec_lot').load("apps/Transaction/QC_scan/button_rec_lot.php?id="+a);
	...
	function ccmt(id,type){
		...
		$('#tampilcmt').html('<img src="./assets/images/spinner.gif">');
 		if (app == 0) {
 			...
 			$('#tampilcmt').load("apps/Transaction/QC_scan/tampilcmt.php");
 			$('#tampilselectlot').load("apps/Transaction/QC_scan/selectlot.php");
			$('#showcmt').val('');
			$('#showcolors').val('');
			$('#rolecmt').val('');
 			$('#idcmt').val('');
			...
			$('#rolecmt').val('');
 			$('#idcmt').val('');
				...
				exdateshow = showexdate[2]+"-"+showexdate[1]+"-"+showexdate[0];
							...
							$('#tampilcmt').load("apps/Transaction/QC_scan/tampilcmt.php?type="+type+"&col="+colors+"&exdate="+exdate+"&cmt="+cmts);
							$('#tampilselectlot').load("apps/Transaction/QC_scan/selectlot.php?id="+id);
							$('#showcmt').val(cmtq);
	...
	function lotnumber(id){
			...
			$.ajax({
				type: 'GET',
				url:  "apps/Transaction/QC_scan/tampillotnumber.php?id="+id+"&cmt="+cmt+"&idc="+idcmt,
				success: function() {
					$('#tampillot').load("apps/Transaction/QC_scan/tampillotnumber.php?id="+id+"&cmt="+cmt+"&idc="+idcmt);
					$('#tampilshade').load("apps/Transaction/QC_scan/selectshade.php?lastrec="+lastrec);
					$('#ceknolot').val(1);
				}
			});
	}
	...
	function correct(lot,user,role,typelot,master_type_process){
		...
		if ($('#qtytotal').val() == $('#qty_process').val()) {
			swal({
				  title: "Are you sure?",
				  text: 'Good:'+$('#qtygood').val()+' Reject:'+$('#qtyreject').val()+' Rework:'+$('#qtyrework').val(),
				  icon: "warning",
				  buttons: true,
				  dangerMode: true,
				})
					...
					$.ajax({
						...
						success: function() {
							swal("Saved!", {
						  	...
						  	//$('#formku').load("apps/Transaction/QC_scan/content_isi.php?id=1");
							window.location.reload();
						}
					});
			...
			swall("Qty Not Match!!","Qty only "+$('#qty_process').val()+" Pcs","error",3000);
			$('#qtyreject').val(0);
			$('#qtyrework').val(0);
	...
	function hitung (val) {
		const total = parseInt($('#qtygood').val())+parseInt($('#qtyrework').val())+parseInt($('#qtyreject').val())+parseInt($('#qtystd').val());
		if (total > $('#qty_process').val()) {
			swall("Qty Not Match!!","Qty only "+$('#qty_process').val()+" Pcs","error",1000);
			$('#qtyreject').val(0);
			$('#qtyrework').val(0);
			$('#qtytotal').val($('#qtygood').val());
			$('#qtygood').val($('#qty_process').val());
			$('#qtystd').val(0);
		} else {
			$('#qtytotal').val(total);
		}
	...
	function newrework(a){
		if(a == 1) {
			$('#gruplotmaking').show();
			$('#nocmt').load('apps/Transaction/QC_scan/choosecmt.php?type='+a);
			$('#tampilcmt').load('apps/Transaction/QC_scan/tampilcmt.php?type='+a);
		} else if (a == 2) {
			$('#gruplotmaking').show();
			$('#nocmt').load('apps/Transaction/QC_scan/choosecmt.php?type='+a);
			$('#tampilcmt').load('apps/Transaction/QC_scan/tampilcmt.php?type='+a);
			...
			$('#process-status').val(a);
					...
					$.ajax({
						...
						success: function() {
							...
							$('#tampilcontent').load("apps/Transaction/QC_scan/content_isi.php?usercode="+encodeURI($("#usercode").val())+"&stat=process");
							$("#lot_no").focus();
						}
					});
		}
	...
	function process_end(a){
		var alltotal = parseInt($('#qtygood').val())+parseInt($('#qtyreject').val())+parseInt($('#qtyrework').val())+parseInt($('#qtystd').val());  
		if($('#qtygood').val() == 'undefined' ){
			swall('Restart Modal','Close and Click Again END','error','2000');
		} else if($('#qtyreject').val() == '0' && $('#qtyrework').val() == '0' && $('#qtygood').val() != $('#qty_process').val() ){
			swall('Qty Not Match','Qty Good tidak sesuai dengan Qty Lot','error','2000');
			$('#qtygood').val($('#qty_process').val());
			$('#qtyreject').val(0);
			$('#qtyrework').val(0);
			$('#qtystd').val(0);
		} else if(parseInt(alltotal) != parseInt($('#qty_process').val())){
			swall('Qty Not Match','Check Your Good Reject Rework Std','error','2000');
			$('#qtygood').val($('#qty_process').val());
			$('#qtyreject').val(0);
			$('#qtyrework').val(0);
			$('#qtystd').val(0);
					...
					$.ajax({
						...
						success: function(lot) {
									...
									$("#lot_no").focus();
										...
										explot = lot.split('|');
										expasl = explot[0].split('_');
										exprwk = explot[1].split('_');
										exprjk = explot[2].split('_');
										expstd = explot[3].split('_');

										swal({
											  icon: 'success',
											  title: 'Lot No : '+ expasl[0] + exprwk[0] + exprjk[0],
											  text: 'Saved',
											  footer: '<a href>Why do I have this issue?</a>'
										}).then((willoke) => {
											if(expasl[0] != ''){
												javascript: window.open("lib/pdf-qrcode.php?c="+expasl[1], "_blank", "width=700,height=450");
											}
											if(exprwk[0] != ''){
												javascript: window.open("lib/pdf-qrcode.php?c="+exprwk[1], "_blank", "width=700,height=450");
											}
											if(exprjk[0] != ''){
												javascript: window.open("lib/pdf-qrcode.php?c="+exprjk[1], "_blank", "width=700,height=450");
											}
											if(expstd[0] != ''){
												javascript: window.open("lib/pdf-qrcode.php?c="+expstd[1], "_blank", "width=700,height=450");
											}
										});
