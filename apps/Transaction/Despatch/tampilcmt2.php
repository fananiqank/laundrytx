...
if ($_GET['typelot'] == 'A'){
	...
	$whererole1 =  "a.role_wo_master_id = '".$rolewomasterid."' AND a.lot_no = '".$_GET['lot']."' AND
					concat(a.lot_no,'_',a.role_wo_id,'_',c.master_process_id) NOT IN 
					(select concat(lot_no,'_',role_wo_id,'_',master_process_id) from laundry_process where process_type = 4) 
					AND 
...
// echo "select $fieldrole1 from $tablerole1 where $whererole1 ORDER by H.role_wo_seq,H.role_dtl_wo_seq limit 1";
...
// 						FROM
// 							laundry_role_wo a
// 							LEFT JOIN ( SELECT role_wo_id, process_qty_good, lot_no FROM laundry_process WHERE role_wo_master_id = '".$rolewomasterid."' AND lot_no = '".$_GET['lot']."' AND process_type = 4 ORDER BY process_type DESC ) AS e ON e.role_wo_id = a.role_wo_id 
// 						WHERE
// 							role_wo_master_id = '".$rolewomasterid."' 
// 							AND concat ( e.lot_no, '_', e.role_wo_id, '_', a.master_type_process_id ) IN ( SELECT concat ( lot_no, '_', role_wo_id, '_', master_type_process_id ) FROM laundry_process WHERE process_type = 4 ) 
// 						ORDER BY
// 							a.role_wo_seq DESC 
// 					   ) as fo order by role_wo_seq";
//====================================================================================================
...
$selstatus = $con->select("laundry_lot_number a 
						   ...
						   GROUP BY role_wo_master_id,role_wo_status,role_wo_id) as c
						    on b.role_wo_master_id = c.role_wo_master_id","COALESCE(role_wo_status,1) as app,b.role_wo_master_id,c.role_wo_id","lot_no = '".$_GET['lot']."' and role_wo_id = '".$role1['role_wo_id']."'");
...
if (count($cmt['wo_no']) == 0){
...
else if ($role1['master_type_process_id'] != '6' && $role1['master_type_process_id'] != ''){
	echo "<script>
	swal({
		 icon: 'error',
		 title: 'Not For Despatch',
	...
	$jmlprocessid = $con->selectcount("laundry_process","process_id","lot_no = '".$_GET['lot']."' and role_wo_master_id = '".$rolewomasterid."' and master_type_process_id = '6' and wo_master_dtl_proc_id = '".$cmt['wo_master_dtl_proc_id']."'");
