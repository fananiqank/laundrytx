...
<?php
	...
	foreach ($selectid as $idu) {}
		$sequence = $idu['max']+1;
	$expmt = explode('/',$cmt_no);
	$selectseq = $con->select("laundry_lot_number","MAX(lot_id) as max","wo_master_dtl_procmaster_dtl_proc_id = '$_GET[idc]' and lot_jenis = '$_GET[id]'");
	foreach ($selectseq as $seq) {}
		$urut = $seq['max']+1;
...
<?php
	...
	foreach ($selectseq as $seq) {}
		$urut = $seq['max']+1;
	$nolb ='L'.$expmt[0].$expmt[4].$expmt[5].$expmt[6].'A'.sprintf('%03s', $urut)."-".$sequence;
?>
	<a href='javascript:void(0)' class='btn btn-primary'><?=$nolb?></a>
	<input type="hidden" name="lotnumb" id="lotnumb" value="<?=$nolb?>">
<?php
