	...
	if ($cmt_no != ''){
		...
		if ($cmt_no != ''){
			$table = "laundry_wo_master_dtl_proc a 
				...
				JOIN (SELECT SUM( rec_qty ) AS wo_master_dtl_proc_qty_rec,wo_master_dtl_proc_id FROM (select wo_no, garment_colors,rec_no,rec_status,rec_qty,wo_master_dtl_proc_id,ex_fty_date,rec_qty_good_upd,rec_qty_std_upd,rec_no_uniq from laundry_receive where wo_no = '$cmt_no' and garment_colors = '$color_no'  GROUP BY wo_no, garment_colors,rec_no,rec_status,rec_qty,wo_master_dtl_proc_id,ex_fty_date,rec_qty_good_upd,rec_qty_std_upd,rec_no_uniq) as ast GROUP BY wo_master_dtl_proc_id) as H
								ON a.wo_master_dtl_proc_id=H.wo_master_dtl_proc_id
				LEFT JOIN (SELECT SUM( rec_qty ) AS rec_manual,wo_master_dtl_proc_id FROM (select wo_no, garment_colors,rec_no,rec_status,rec_qty,wo_master_dtl_proc_id,ex_fty_date,rec_qty_good_upd,rec_qty_std_upd,rec_no_uniq from laundry_receive where wo_no = '$cmt_no' and garment_colors = '$color_no' and rec_type = 1 GROUP BY wo_no, garment_colors,rec_no,rec_status,rec_qty,wo_master_dtl_proc_id,ex_fty_date,rec_qty_good_upd,rec_qty_std_upd,rec_no_uniq) as ast GROUP BY wo_master_dtl_proc_id) as k
								ON a.wo_master_dtl_proc_id=H.wo_master_dtl_proc_id
				...
				LEFT JOIN (select wo_master_dtl_proc_id,lot_no,lot_qty,create_type,role_wo_name_seq from laundry_lot_number group by wo_master_dtl_proc_id,lot_no,lot_qty,create_type,role_wo_name_seq,wo_no,garment_colors) e on a.wo_master_dtl_proc_id = e.wo_master_dtl_proc_id and e.create_type = 1 and role_wo_name_seq = '".$_GET[seqlot]."' 
				LEFT JOIN (select sum(process_qty_good) as qty_good,wo_master_dtl_proc_id from (select wo_master_dtl_proc_id,process_qty_total,master_type_process_id,master_process_id,process_type,max(process_createdate) as process_createdate,role_wo_id,role_dtl_wo_id,COALESCE(process_qty_good,0) as process_qty_good,COALESCE(process_qty_reject,0) as process_qty_reject,COALESCE(process_qty_repair,0) as process_qty_repair,COALESCE(process_qty_std,0) as process_qty_std,wo_no,garment_colors,role_wo_name_seq,role_wo_master_id from laundry_process where master_type_process_id = '2' and wo_no = '".$cmt_no."' and  garment_colors = '".$color_no."' 
				           and role_wo_id = '".$_GET[rolewoid]."' GROUP BY lot_no, wo_master_dtl_proc_id,process_qty_total,master_type_process_id,master_process_id,process_type,role_wo_id,role_dtl_wo_id,COALESCE(process_qty_good,0),COALESCE(process_qty_reject,0),COALESCE(process_qty_repair,0),COALESCE(process_qty_std,0),wo_no,garment_colors,role_wo_name_seq,role_wo_master_id) as jo GROUP BY wo_master_dtl_proc_id) as f ON a.wo_master_dtl_proc_id = f.wo_master_dtl_proc_id
				LEFT JOIN (select count(a.ticketid) cut_qty,wo_no, color,ex_fty_date from qrcode_ticketing_master a JOIN qrcode_ticketing_detail b on a.ticketid=b.ticketid where wo_no = '".$cmt_no."' and color = '".$color_no."' and DATE(ex_fty_date) = '".$ex_fty_date."' and washtype = 'Wash' GROUP BY wo_no, color,ex_fty_date) as g on a.wo_no = g.wo_no and a.garment_colors = g.color and a.ex_fty_date=g.ex_fty_date

				";
			$field = "a.wo_no,a.buyer_po_number,a.wo_master_dtl_proc_qty_md,a.garment_colors,b.buyer_id,H.wo_master_dtl_proc_qty_rec,firstdate,lastdate,COALESCE(SUM(e.lot_qty),0) as totallot,f.qty_good,c.rec_status,DATE(a.ex_fty_date) as ex_fty_date, to_char(a.ex_fty_date, 'DD-MM-YYYY') as ex_fty_date_show,COALESCE(cut_qty,0) + COALESCE(k.rec_manual,0) as cutting_qty,a.color_wash";
			$where = "wo_master_dtl_proc_status = 1 ".$cmtno ."
				GROUP BY A.wo_no,A.buyer_po_number,a.wo_master_dtl_proc_qty_md,A.garment_colors,b.buyer_id,
					cut_qty,H.wo_master_dtl_proc_qty_rec,firstdate,lastdate,f.qty_good,rec_status,a.ex_fty_date,a.color_wash,k.rec_manual";
...
<?php 
	...
	if ($cmt_no != ''){
		...
		if ($cmt_no != ''){
			$table ="laundry_wo_master_dtl_proc a 
					 JOIN (select wo_no,buyer_id from laundry_data_wo GROUP BY wo_no,buyer_id) b on a.wo_no=b.wo_no
					 JOIN (select sum(lot_qty) as jmlqty,b.wo_master_dtl_proc_id from (select wo_master_dtl_proc_id,lot_no,lot_qty,create_type,role_wo_name_seq,lot_type from laundry_lot_number group by wo_master_dtl_proc_id,lot_no,lot_qty,create_type,role_wo_name_seq,wo_no,garment_colors,lot_type) a join laundry_wo_master_dtl_proc b on a.wo_master_dtl_proc_id = b.wo_master_dtl_proc_id where wo_master_dtl_proc_status = 2 and lot_type = 'M' GROUP BY b.wo_master_dtl_proc_id ORDER BY rework_seq DESC) as c 
						on a.wo_master_dtl_proc_id=c.wo_master_dtl_proc_id
					 LEFT JOIN (select sum(process_qty_good) as qty_good,A.wo_master_dtl_proc_id from (select wo_master_dtl_proc_id,process_qty_total,master_type_process_id,master_process_id,process_type,max(process_createdate) as process_createdate,role_wo_id,role_dtl_wo_id,COALESCE(process_qty_good,0) as process_qty_good,COALESCE(process_qty_reject,0) as process_qty_reject,COALESCE(process_qty_repair,0) as process_qty_repair,COALESCE(process_qty_std,0) as process_qty_std,wo_no,garment_colors,role_wo_name_seq,role_wo_master_id,lot_type from laundry_process where master_type_process_id = '2' and wo_no = '".$cmt_no."' and  garment_colors = '".$color_no."' 
				           and role_wo_id = '".$_GET[rolewoid]."' GROUP BY lot_no, wo_master_dtl_proc_id,process_qty_total,master_type_process_id,master_process_id,process_type,role_wo_id,role_dtl_wo_id,COALESCE(process_qty_good,0),COALESCE(process_qty_reject,0),COALESCE(process_qty_repair,0),COALESCE(process_qty_std,0),wo_no,garment_colors,role_wo_name_seq,role_wo_master_id,lot_type) as a join laundry_wo_master_dtl_proc b on a.wo_master_dtl_proc_id=b.wo_master_dtl_proc_id where master_type_process_id = '2' and b.wo_no = '$cmt_no' and b.garment_colors = '$color_no' and DATE(b.ex_fty_date) = '$ex_fty_date' and role_wo_id = '".$_GET[rolewoid]."' and a.lot_type = 2 AND b.wo_master_dtl_proc_status = 2 GROUP BY a.wo_master_dtl_proc_id) as f ON a.wo_master_dtl_proc_id = f.wo_master_dtl_proc_id
					 LEFT JOIN (select wo_master_dtl_proc_id,lot_no,lot_qty,create_type,role_wo_name_seq from laundry_lot_number group by wo_master_dtl_proc_id,lot_no,lot_qty,create_type,role_wo_name_seq,wo_no,garment_colors) e on a.wo_master_dtl_proc_id = e.wo_master_dtl_proc_id and e.create_type = 6  and role_wo_name_seq = '".$_GET[seqlot]."'
					 LEFT JOIN (select sum(process_qty_good) as process_qty_good,wo_master_dtl_proc_id from (select wo_master_dtl_proc_id,process_qty_total,master_type_process_id,master_process_id,process_type,max(process_createdate) as process_createdate,role_wo_id,role_dtl_wo_id,COALESCE(process_qty_good,0) as process_qty_good,COALESCE(process_qty_reject,0) as process_qty_reject,COALESCE(process_qty_repair,0) as process_qty_repair,COALESCE(process_qty_std,0) as process_qty_std,wo_no,garment_colors,role_wo_name_seq,role_wo_master_id from laundry_process where master_type_process_id = '2' and wo_no = '".$cmt_no."' and  garment_colors = '".$color_no."' 
				           and role_wo_id = '".$_GET[rolewoid]."' GROUP BY lot_no, wo_master_dtl_proc_id,process_qty_total,master_type_process_id,master_process_id,process_type,role_wo_id,role_dtl_wo_id,COALESCE(process_qty_good,0),COALESCE(process_qty_reject,0),COALESCE(process_qty_repair,0),COALESCE(process_qty_std,0),wo_no,garment_colors,role_wo_name_seq,role_wo_master_id) as jo where master_type_process_id = '3' GROUP BY wo_master_dtl_proc_id) as d on a.wo_master_dtl_proc_id=d.wo_master_dtl_proc_id
					";
