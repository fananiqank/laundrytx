...
if (strtoupper(strtoupper(strtoupper($_GET['typelot']))) == 'A'){
	$role1lot =  strtoupper($_GET['lot']) ;
		foreach ($con->select(" laundry_wo_master_dtl_proc a 
		                        ...
		                        ",
		                        "rec_no = '".strtoupper($_GET['lot'])."' limit 1
		                         ") as $cmt) {}
...
foreach ($con->select("(
						...
						FROM
							...
							LEFT JOIN ( SELECT role_dtl_wo_id, process_qty_good,lot_no,
										role_wo_id FROM laundry_process WHERE role_wo_master_id = '".$rolewomasterid."' AND lot_no = '". strtoupper($_GET['lot']) ."' AND process_type = 4 ORDER BY process_type DESC ) AS e ON e.role_dtl_wo_id = b.role_dtl_wo_id 
						WHERE
