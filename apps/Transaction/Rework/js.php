<script>
	...
	function pindahData2(cmt,color,exdate,usercode){
	 	//CMT
 		trimcmt = cmt.trim();
		...
		expcolor = trimcolor.split('#');
	 	...
	 	if (cmt == ''){
			...
			$('#contentwip').load("apps/Transaction/Rework/tampilcmt2.php?cm="+cmt+"&co="+color+"&exdate="+exdate);
			$('#seq_pro').load("apps/Transaction/Rework/selectsequence.php?cm="+cmt+"&co="+color+"&exdate="+exdate);
			$('#wo_no').val(cmt);
			$('#color_no').val(color);
			$('#ex_fty_date').val(exdate);
			$('#usercode').val(usercode);
			document.getElementById("totalpcs").readOnly = false; 
			$('#loader-off').click();
		}
	}
				...
				if(ui == '1'){
					...
					.then((willDelete) => {
					  if (willDelete) {
						...
						$('#tampilwip1').load("apps/Transaction/Rework/tampilswip.php?&cm="+$('#wo_no').val()+"&co="+$('#color_no').val()+"&exdate="+$('#ex_fty_date_asli').val());
...
//jika success tersimpan
	...
	function reclot(a){
		$('#formku').load("apps/Transaction/Rework/content_isi.php?id="+a);
		$('#button_rec_lot').load("apps/Transaction/Rework/button_rec_lot.php?id="+a);
	...
	function cekpcs(a){
			if(parseInt(a) > parseInt($('#sisalotout').val())){
				swall('Qty more than balance','Qty Lot More than Balance Qty Lot','info','2000');				
				$('#totalpcs').val('');
				$('#totalpcs').focus();
			} 
