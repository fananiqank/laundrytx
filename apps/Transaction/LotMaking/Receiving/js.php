...
expcolor4 = expcolor3.split('/');
	...
	(function( $ ) {
		...
		var datatableInit = function() {
			...
			$table.dataTable({
				"processing": true,
	         	"serverSide": true,
	        	"ajax": "apps/Transaction/Receiving/data.php",
	...
	function viewdata(a){
		$('#view').val(a);
		javascript: window.location.href="content.php?option=View&task=receive&act=ugr_view";
	}
	
	function conf(cmt,colors,exdate){
	if ($('#nocmt').val() == '' || $('#nocolor').val() == '' || $('#exftydate').val() == ''|| $('#noinseams').val() == '' || $('#nosizes').val() == '' || $('#qty_in').val() == ''){
		swall('Data Not Complete','Check Data WO No,Colors,Ex_fty_date,Inseams,Sizes,Qty','error','2000');
		...
		if (cmt != $('#wo_no').val() && colors != $('#garment_colors').val() && $('#exftydate').val() != $('#ex_fty_date').val()){
			swall('WO No Colors Ex Fty Date Not Same','WO No Colors Ex Fty Date must same','error','2000');
	...
	function saverec(){
		if ($('#username_1').val() == ''){
				swall('Fill User','Must Fill the User','info','2000');
		} else {
				//if ($('#idcek').val() > 0) {
				if ($('#balance_1').val() <= 0) {
					if($('#idlast').val() == 1){
						...
						swall('Can not Submit!!','Receive Qty More than Cutting Qty','info','2000');
					} else {
						swal({
						  title: "Are You Sure?",
						  text: "Save Receiving",
											...
											success: function(lot) {
												swal({
												  ...
												  footer: '<a href>Why do I have this issue?</a>'
													 ...
													 javascript: window.location.href="content.php?"+$('#getpmodcart').val();
												});
		        ...
		        $("#nocmt").autocomplete({  
		         	source: function(request, response) {
			            $.getJSON(
			                ...
			                { d:'1', term:request.term, scolor:$('#nocolor').val(), sins:$('#noinseams').val(), ssiz:$('#nosizes').val(), exdate:$('#exftydate').val() }, 
			                response
			            );
			        },
		           	minLength:2, 
	...
	$(function() {  
		        $("#nocolor").autocomplete({
		         	source: function(request, response) {
			            $.getJSON(
			                "apps/Transaction/Receiving/sourcedata.php",
			                { d:'2', term:request.term, scmt:$('#nocmt').val(), sins:$('#noinseams').val(), ssiz:$('#nosizes').val(), exdate:$('#exftydate').val() }, 
			                response
			            );
			        },  
		           	minLength:0, 
	...
	$(function() {  
		        ...
		        $("#noinseams").autocomplete({
		         	source: function(request, response) {
			            $.getJSON(
			                "apps/Transaction/Receiving/sourcedata.php",
			                { d:'3', term:request.term, scmt:$('#nocmt').val(), scolor:$('#nocolor').val(), ssiz:$('#nosizes').val(), exdate:$('#exftydate').val() }, 
			                response
			            );
			        }, 
		           	minLength:0, 
	...
	$(function() {  
		        ...
		        $("#nosizes").autocomplete({
		         	source: function(request, response) {
			            $.getJSON(
			                "apps/Transaction/Receiving/sourcedata.php",
			                { d:'4', term:request.term, scmt:$('#nocmt').val(), sins:$('#noinseams').val(), scolor:$('#nocolor').val(), exdate:$('#exftydate').val() }, 
			                response
			            );
			        }, 
		           	minLength:0, 
	...
	$(function() {  
		        $("#exftydate").autocomplete({
		         	source: function(request, response) {
			            $.getJSON(
			                "apps/Transaction/Receiving/sourcedata.php",
			                { d:'5', term:request.term, scmt:$('#nocmt').val(), scolor:$('#nocolors').val(), ssiz:$('#nosizes').val(), sins:$('#noinseams').val() }, 
			                response
			            );
			        },  
		           	minLength:0, 
	...
	function hitungin(a,id){
		...
		if (parseInt($('#qtyin_'+id).val()) > parseInt($('#qtysend_'+id).val())){
			swall('Qty Over','Receive Qty More than Send Qty','error','2000');
