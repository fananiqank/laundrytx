...
if($_GET['type'] == 1) {
	if ($cmt_no != ''){
		$cmtno = "and a.wo_no = '".$cmt_no."' and a.garment_colors = '".$color_no."' and DATE(a.ex_fty_date) = '".$ex_fty_date."'";
	} else {
		$cmtno = "";
	}
		if ($cmt_no != ''){
			$table = "laundry_wo_master_dtl_proc a 
     JOIN (select trim(wo_no) wo_no,buyer_id from qrcode_workorders GROUP BY wo_no,buyer_id) b on a.wo_no=b.wo_no
		 JOIN (select count(a.ticketid) cut_qty,wo_no, trim(color) color,ex_fty_date from qrcode_ticketing_master a JOIN qrcode_ticketing_detail b on a.ticketid=b.ticketid where wo_no = '".$cmt_no."' and trim(color) = '".$color_no."' and DATE(ex_fty_date) = '".$ex_fty_date."' and washtype = 'Wash' GROUP BY wo_no, color,ex_fty_date) as g on a.wo_no = g.wo_no and a.garment_colors = g.color and a.ex_fty_date=g.ex_fty_date 
		 LEFT JOIN (select sum(process_qty_good) as qty_good,wo_master_dtl_proc_id from (select wo_master_dtl_proc_id,process_qty_total,master_type_process_id,master_process_id,process_type,max(process_createdate) as process_createdate,role_wo_id,role_dtl_wo_id,COALESCE(process_qty_good,0) as process_qty_good,COALESCE(process_qty_reject,0) as process_qty_reject,COALESCE(process_qty_repair,0) as process_qty_repair,COALESCE(process_qty_std,0) as process_qty_std,wo_no,garment_colors,role_wo_name_seq,role_wo_master_id from laundry_process where master_type_process_id = '2' and wo_no = '".$cmt_no."' and garment_colors = '".$color_no."' and role_wo_id = '".$_GET[rolewoid]."' GROUP BY lot_no, wo_master_dtl_proc_id,process_qty_total,master_type_process_id,master_process_id,process_type,role_wo_id,role_dtl_wo_id,COALESCE(process_qty_good,0),COALESCE(process_qty_reject,0),COALESCE(process_qty_repair,0),COALESCE(process_qty_std,0),wo_no,garment_colors,role_wo_name_seq,role_wo_master_id) as jo GROUP BY wo_master_dtl_proc_id) as f ON a.wo_master_dtl_proc_id = f.wo_master_dtl_proc_id";

			$field = "a.wo_no,a.buyer_po_number,a.wo_master_dtl_proc_qty_md,a.garment_colors,b.buyer_id,(select COALESCE(sum(qrcodebatch_qty),0) from laundry_qrcodebatch where wo_master_dtl_proc_id=a.wo_master_dtl_proc_id) as wo_master_dtl_proc_qty_rec,(select min(rec_createdate) from laundry_receive where wo_master_dtl_proc_id=a.wo_master_dtl_proc_id GROUP BY wo_master_dtl_proc_id) firstdate,(select max(rec_status) from laundry_receive where wo_master_dtl_proc_id=a.wo_master_dtl_proc_id GROUP BY wo_master_dtl_proc_id) rec_status,(select COALESCE(SUM(lot_qty),0) from laundry_lot_number where wo_master_dtl_proc_id = a.wo_master_dtl_proc_id and create_type = 1 and role_wo_name_seq = '".$_GET[seqlot]."' ) totallot,f.qty_good,DATE(a.ex_fty_date) as ex_fty_date, to_char(a.ex_fty_date, 'DD-MM-YYYY') as ex_fty_date_show,COALESCE(cut_qty,0) as cutting_qty,a.color_wash ";
