...
//input scan Receive confirm
	if ($_POST['confirm'] == '1') {
		...
		$selwodtlproc = $con->select("laundry_wo_master_dtl_proc", "wo_master_dtl_proc_id,role_wo_master_id", "wo_no = '" . $wo_no . "' and garment_colors = '" . $colors . "' and DATE(ex_fty_date) = '" . $exdate . "' and wo_master_dtl_proc_status = 1");
		...
		if($selcountreceive == 0){
			...
			if($_POST['willrec'] <> ''){$willrec = $_POST['willrec'];}
			else{$willrec = 0;}
			
			$dataproc = array(
				'wo_master_dtl_proc_qty_rec' => $willrec,
				'cutting_qty' => $_POST['cutqty'],
				'wo_master_dtl_proc_modifydate' => $date,
				'wo_master_dtl_proc_modifyby' => $_SESSION['ID_LOGIN'],
				//'rework_seq' => 1,
			);
			$execproc = $con->update("laundry_wo_master_dtl_proc", $dataproc, "wo_no = '" . $wo_no . "' and garment_colors = '" . $colors . "' and DATE(ex_fty_date) = '" . $exdate . "' and wo_master_dtl_proc_status = 1");

			//end wo master dtl proc =======================================================

			//input data wo master & wo master detail
			$selwodtl = $con->select("laundry_scan_qrcode", "wo_no,garment_colors,garment_inseams,garment_sizes,SUM(scan_qty) as totqty", "trim(wo_no) = trim('" . $wo_no . "') and trim(garment_colors) = trim('" . $colors . "') and scan_status = '0' and scan_type = 1 GROUP BY wo_no,garment_colors,garment_inseams,garment_sizes");
			foreach ($selwodtl as $wodtl) {
				//cek wo sudah ada pada master wo laundry
			$countwodtl = $con->selectcount("laundry_wo_master_dtl", "wo_master_dtl_id", "wo_no = '" . $wo_no . "' and garment_colors = '" . $colors . "' and garment_inseams = '" . $wodtl['garment_inseams'] . "' and garment_sizes= '" . $wodtl['garment_sizes'] . "'");
				...
				if($countwodtl > 0) {
					...
					$execwodtl = $con->update("laundry_wo_master_dtl", $datawodtl,"wo_no = '" . $wo_no . "' and garment_colors = '" . $colors . "' and garment_inseams = '" . $wodtl['garment_inseams'] . "' and garment_sizes= '" . $wodtl['garment_sizes'] . "'");
				} else {
			...
			//update status dari scan_qrcode
				$selscanzero = $con->select("laundry_scan_qrcode","scan_id","trim(wo_no) = trim('" . $wo_no . "') and trim(garment_colors) = trim('" . $colors . "') and scan_status = '0' and scan_type = 1 limit '".$_POST[recnow]."'");
