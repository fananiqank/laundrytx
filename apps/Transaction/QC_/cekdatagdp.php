...
session_start();
	...
	if ($tiketmaster > 0){
		...
		if ($_POST['scan_type'] == '3'){
			...
			foreach ($con->select("qrcode_step_detail","workcenter_id,workcenter_seq","step_id_detail = 6") as $mystepdetail) {} 
			
			$beforestepdetail = $mystepdetail['workcenter_seq']-2;
			...
			//cek apakah data ini sudah di input pada process sebelumnya. berstatus 1 (confirm)
				...
				$scannow = $con->selectcount("laundry_scan_qrcode","scan_qrcode","scan_qrcode = '$_POST[scanner]' and scan_status = 0");
