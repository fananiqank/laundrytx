...
if ($_GET['d'] == 1){
	...
	$dat=$con->select("laundry_wo_master_dtl_proc",
					   "wo_no",
					   "wo_no ilike '%$term%' and wo_master_dtl_proc_status = 1 GROUP BY wo_no");
	...
	$term = trim(strip_tags($_GET['term'])); 
	$dat=$con->select("laundry_wo_master_dtl_proc",
					   "garment_colors","garment_colors ilike '%$term%' and wo_master_dtl_proc_status = 1 and wo_no = '$scmt' GROUP BY garment_colors");
	...
	$term = trim(strip_tags($_GET['term'])); 
	$dat=$con->select("laundry_wo_master_dtl_proc","DATE(ex_fty_date) as ex_fty_date,to_char(ex_fty_date,'DD-MM-YYYY') as ex_fty_date_show","DATE(ex_fty_date)::text ilike '%$term%' and wo_master_dtl_proc_status = 1 $cmt $color GROUP BY ex_fty_date");
