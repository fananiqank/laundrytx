<script>
   ...
   //      var spintable = $('#datatable-ajax2').DataTable({
	...
	$(function() {  
        $("#ex_fty_date").autocomplete({
         	source: function(request, response) {
	            $.getJSON(
	                "apps/Transaction/Receiving/sourcedata.php",
	                { d:'5', term:request.term, scmt:$('#cmt_no').val(), scolor:$('#color_no').val()}, 
	                response
	            );
	        }, 
           	minLength:0, 
	...
	$(function() {  
		$("#color_no").autocomplete({
		   ...
		   source: function(request, response) {
			    $.getJSON(
			        "apps/Transaction/Receiving/sourcedata.php",
			        { d:'4', term:request.term, scmt:$('#cmt_no').val()}, 
			        response
			    );
			},
		    minLength:0, 
	...
	function viewdata(a) {
		...
		// if ($('#wo_no').val()=="" || $('#color_no').val()=="" || $('#ex_fty_date').val() =="" || $('#inseam').val() =="" || $('#sz').val() =="" || $('#rcv_qty').val() =="") {
		if ($('#wo_no').val()=="" || $('#color_no').val()=="" || $('#ex_fty_date').val() =="" || $('#rcv_qty').val() =="") {
				swal({
					icon: 'error',
					title: 'Isi',
					text: 'Fill CMT / Color / EX FTY Date / inseam / Size / Qty',
					timer: 2000,
				});
		} else {
			...
			$.ajax({
				type: 'POST',
				url: "apps/Transaction/Receiving/addreceive.php",
				data: data,
	...
	function hapuscart(a) {
		...
		$.ajax({
			type: 'POST',
			url: "apps/Transaction/Receiving/remreceive.php?id=" + a,
			data: data,
			success: function() {
	...
	function saverec() {
		if ($('#username_1').val() == '') {
			...
			$.ajax({
				type: 'POST',
				url: "apps/Transaction/Receiving/simpan_receive.php?user="+$('#username_1').val(),
				data: data,
				success: function(lot) {
					...
					javascript: window.location.href = "content.php?option=Transaction&task=receive&act=ugr_transaction";
				}
			});
