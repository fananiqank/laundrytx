	...
	if ($_POST['codeproc'] == 'return') {
		...
		// $idwipdtl = $con->idurut("laundry_wo_master_dtl", "wo_master_dtl_id");
		// $datarolemaster = array(
		// 	'wo_master_dtl_id' => $idwipdtl,
		// 	'wo_no' => $_POST['nocmt'],
		// 	'garment_colors' => $_POST['nocolor'],
		// 	'garment_inseams' => $_POST['noinseams'],
		// 	'garment_sizes' => $_POST['nosizes'],
		// 	'wo_master_dtl_qty_rec' => $_POST['qty_in'],
		...
		foreach ($_POST['cutqty'] as $key => $value) {
			...
			if ($jmlwo == '0') {
				$idwo = $con->idurut("laundry_wo_master", "wo_master_id");
				$selwork = $con->select("work_orders", "buyer_style_no,wo_qty", "wo_no = '$wono'");
		...
		$selrolechild = $con->select(
			...
			"role_wo_master_id = '$wodtlproc[role_wo_master_id]' and 
									  role_wo_seq <= (select role_wo_seq from laundry_role_wo where role_wo_master_id = '$wodtlproc[role_wo_master_id]' and master_type_process_id = 2 and role_wo_name_seq = 1)",
			"role_wo_seq"
		);
