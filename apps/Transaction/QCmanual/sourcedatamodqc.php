...
//select role wo id
foreach ($con->select("laundry_lot_number a 
							 join laundry_role_child b on a.role_wo_master_id=b.role_wo_master_id",
							"b.role_wo_id",
							"a.lot_no = '$_GET[lot]'and role_child_status = 0 and b.lot_type = 2",
							" b. role_wo_seq","1")as $roleid){}
...
<?php 
	...
	?>
		<tr>
			...
			<th width="3%"><a href="javascript:void(0)" onclick="hapusqckeranjang('<?=$_GET[lot]?>','<?=$grm[qc_keranjang_type]?>')"><i class="fa fa-trash"></a></th>
		</tr>
	<?php 
		  $no++; 
...
<tr style="font-size: 16px;">
	...
	<th width="5%" style="text-align: center"><b><?php echo $supertotalall; ?></b>
		...
		<input type="hidden" id="rolewoidmod" name="rolewoidmod" value="<?=$roleid[role_wo_id]?>">
	</th>
