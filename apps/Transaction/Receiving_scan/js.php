...
if($('#nocmt').val() != ''){
    ...
    //     processing: true,
    //     serverSide: true,
    //     ajax: 'apps/Transaction/Receiving_scan/data.php?cm='+encodeURI("<?=$_GET[cm]?>")+'&co='+encodeURI("<?=$_GET[co]?>")
    // });
	$(document).ready(function(){
       // tampil_data_barang();   //pemanggilan fungsi tampil barang.
        $('#datatable-ajax2').DataTable({
        		// "ajax": "apps/Transaction/Receiving_scan/data.php?cm="+encodeURI($('#nocmt').val())+"&co="+encodeURI($('#nocolor').val()),
        		ajax: 'apps/Transaction/Receiving_scan/data.php?cm='+encodeURI($('#nocmt').val())+'&co='+encodeURI(colors),
				processing: true,
				serverSide: true,
				order: [[6, 'desc']],
			});
		...
		expcolor4 = expcolor3.split('/');
	 	...
	 	if (cmt == '') {
	 		...
	 		swal({
	 		 ...
	 		 var hal = $('#getp').val();
			 window.location="content.php?"+hal+"&cm="+encodeURI(cmt)+"&co="+encodeURI(colors);
		}
	}
