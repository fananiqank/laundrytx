...
if ($_GET['sstatus'] == 1){
	...
	$statcolor2 = "and CONCAT(A.wo_no,'_',garment_colors) NOT IN (select CONCAT(wo_no,'_',garment_colors) from laundry_wo_master_dtl_proc where wo_no = '$scmt' and wo_master_dtl_proc_status = 1)";
	...
	$statcolor2 = "and CONCAT(A.wo_no,'_',garment_colors) IN (select CONCAT(wo_no,'_',garment_colors) from laundry_wo_master_dtl_proc where wo_no = '$scmt' and wo_master_dtl_proc_status = 1)";
}

if ($_GET['d'] == 1) {
	$term = trim(strip_tags($_GET['term'])); 
	$dat=$con->select("laundry_data_wo A LEFT JOIN qrcode_cutplan_dtlhead B ON A.wo_no = B.cpdwono",
					   "buyer_id",
...
// }
	...
	// }
	$term = trim(strip_tags($_GET['term'])); 
	$cekwoqr = $con->selectcount("qrcode_cutplan_dtlhead","cpdwono","cpdwono like '%$scmt%'");
	if($cekwoqr > 0) {
		...
		$dat=$con->select("laundry_data_wo A $joinvip",
					   "garment_colors $colwash","garment_colors ilike '%$term%' $buyer $cmt $style $statcolor2 
					   and concat(A.wo_no,'_',garment_colors) NOT IN (select concat(wo_no,'_',garment_colors) from laundry_wo_master_keranjang)
					   GROUP BY garment_colors $colwash");
		// echo "select garment_colors $colwash from laundry_data_wo A $joinvip where garment_colors ilike '%$term%' $buyer $cmt $style $statcolor2 
		// 			   and concat(A.wo_no,'_',garment_colors) NOT IN (select concat(wo_no,'_',garment_colors) from laundry_wo_master_keranjang)
		// 			   GROUP BY garment_colors $colwash";
	}
	foreach($dat as $row)
