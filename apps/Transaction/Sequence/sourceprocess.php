...
<table width="100%" cellpadding="0" cellspacing="0" class="table-wrapper-scroll-y my-custom-scrollbar">
	...
	<tr>
		<td width="80%">
			<input type="checkbox" class="control-primary" name="process[]" id="process_<?php echo $subcat[master_process_id]; ?>" value="<?=$subcat['master_process_id']?>" onchange="onc(<?php echo $subcat['master_process_id']; ?>)">&ensp;<?php echo $subcat['master_process_name']; ?><br>
		</td>
		<td width="20%">
			<input id="urseq_<?php echo $subcat[master_process_id]; ?>" name="urseq[]" style="width:50%" class="form-control" type="text" disabled="disabled">
		</td>
	</tr>
<?php 
