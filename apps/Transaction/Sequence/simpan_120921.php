...
//echo "1";
	...
	if($_POST['getidm'] != ''){
			$selcount = $con->select("laundry_role_wo","COUNT(role_wo_id) as jmlrole","role_wo_createdby = '$_SESSION[ID_LOGIN]' and role_wo_master_id = '$_POST[getd]'");
			
			foreach($selcount as $count) {}
			$selnama = $con->select("laundry_role_wo","COUNT(role_wo_id) as jmlrole","role_wo_createdby = '$_SESSION[ID_LOGIN]' and master_type_process_id = '$exptype[0]' and role_wo_master_id = '$_POST[getd]'");
	...
	//input new sequence pada pembuatan new sequence
			...
			$selcount = $con->select("laundry_role_grup","COUNT(role_grup_id) as jmlrole","role_grup_createdby = '$_SESSION[ID_LOGIN]' and role_grup_master_id = '$idrolemaster'");
			
			foreach($selcount as $count) {}
			$selnama = $con->select("laundry_role_grup","COUNT(role_grup_id) as jmlrole","role_grup_createdby = '$_SESSION[ID_LOGIN]' and master_type_process_id = '$exptype[0]' and role_grup_master_id = '$idrolemaster'");
			foreach($selnama as $nam) {}
...
// echo "4";
	...
	if ($_POST['sub'] != ''){
		...
		if ($naik['naik'] != '' && $_POST['turun'] == ''){
			$selseq = $con->select("laundry_role_dtl_wo","*","role_dtl_wo_id = '$_POST[idnyaedit]'");
			foreach ($selseq as $seq) {}
			$dataseq = array(
				'role_dtl_wo_seq' => $naik,
			);
			$execseq = $con->update("laundry_role_dtl_wo",$dataseq,"role_dtl_wo_id = '$_POST[idnyaedit]'");
			$selsequ = $con->select("laundry_role_dtl_wo","*","role_dtl_wo_seq = '$naik' and role_dtl_wo_id != '$_POST[idnyaedit]' and role_wo_id = '$_POST[sub]'");
			foreach ($selsequ as $sequ) {}
			...
			foreach ($selseq as $seq) {}
			$dataseq = array(
				'role_dtl_wo_seq' => $turun,
			);
			$execseq = $con->update("laundry_role_dtl_wo",$dataseq,"role_dtl_wo_id = '$_POST[idnyaedit]'");
			$selsequ = $con->select("laundry_role_dtl_wo","*","role_dtl_wo_seq = '$turun' and role_dtl_wo_id != '$_POST[idnyaedit]' and role_wo_id = '$_POST[sub]'");
			foreach ($selsequ as $sequ) {}
				$seqdb = $sequ['role_dtl_wo_seq']-'1';
				$datasequ = array(
				'role_dtl_wo_seq' => $seqdb,
		...
		if ($_GET['ed'] == 1){
			...
			$execseq = $con->update("laundry_role_wo",$dataseq,"role_wo_id = '$_POST[idnyaedit]'");
			$selsequ = $con->select("laundry_role_wo","*","role_wo_seq = '$naik' and role_wo_id != '$_POST[idnyaedit]' and role_wo_master_id = '$seq[role_wo_master_id]'");
			foreach ($selsequ as $sequ) {}
				$seqdb = $sequ['role_wo_seq']+'1';
				$datasequ = array(
				'role_wo_seq' => $seqdb,
				);
				$execsequ = $con->update("laundry_role_wo",$datasequ,"role_wo_id = '$sequ[role_wo_id]'");

		//update role child
			//select lot no pada role child
			$sellotchildnaik = $con->select("laundry_role_child","role_child_id,role_child_status","role_wo_id = '".$sequ[role_wo_id]."' and role_wo_master_id = '$seq[role_wo_master_id]' and role_child_status = 0");
			foreach ($sellotchildnaik as $lotchildnaik) {
				
						///update sequence
							$datachnaik = array(
								'role_wo_seq' => $naik,
							);
							$execchnaik = $con->update("laundry_role_child",$datachnaik,"role_child_id = '$lotchildnaik[role_child_id]'");

					//ubah sequence / menyesuaikan sequence
						$selchildseqnaik = $con->select("laundry_role_child","role_child_id,role_wo_seq","role_wo_seq >= '".$naik."' and role_wo_id != '$_POST[idnyaedit]' and role_wo_master_id = '$seq[role_wo_master_id]'");
						// echo "select role_child_id,role_wo_seq from laundry_role_child where role_wo_seq >= '".$naik."' and role_wo_id != '$_POST[idnyaedit]' and role_wo_master_id = '$seq[role_wo_master_id]'";
						foreach ($selchildseqnaik as $cseqnaik) {
							$newseqnaik = $cseqnaik['role_wo_seq']+1;
							$dataseqnaik = array(  
							 'role_wo_seq' => $newseqnaik,
							); 
							//var_dump($dataseqnaik);
							$execseqnaik = $con->update("laundry_role_child",$dataseqnaik,"role_child_id = $cseqnaik[role_child_id]");
							//var_dump($execseq);
						}
					//end ubah sequence
			...
			$execseq = $con->update("laundry_role_wo",$dataseq,"role_wo_id = '$_POST[idnyaedit]'");
			$selsequ = $con->select("laundry_role_wo","*","role_wo_seq = '$turun' and role_wo_id != '$_POST[idnyaedit]' and role_wo_master_id = '$seq[role_wo_master_id]'");
			...
			//update role child
				//select lot no pada role child
				$sellotchild = $con->select("laundry_role_child","role_child_id,role_child_status","role_wo_id = '".$sequ[role_wo_id]."' and role_wo_master_id = '$seq[role_wo_master_id]' and role_child_status = 0");
				foreach ($sellotchild as $lotchild) {
							...
							///update sequence
								...
								$execchturun = $con->update("laundry_role_child",$datachturun,"role_child_id = '$lotchild[role_child_id]'");

						//ubah sequence / menyesuaikan sequence
							$selchildseq = $con->select("laundry_role_child","role_child_id,role_wo_seq","role_wo_seq >= '".$turun."' and role_wo_id != '$_POST[idnyaedit]' and role_wo_master_id = '$seq[role_wo_master_id]'");
							foreach ($selchildseq as $cseq) {
								$newseq = $cseq['role_wo_seq']-1;
								$dataseq = array(  
								 'role_wo_seq' => $newseq,
								); 

								$execseq = $con->update("laundry_role_child",$dataseq,"role_child_id = '$cseq[role_child_id]'");
							}
						//end ubah sequence
				}
		}
...
// echo "11";
			...
			$selsequ = $con->select("laundry_role_grup","*","role_grup_seq > '$_GET[e]' and role_grup_createdby = '$_GET[f]'");
...
// echo "12";
	...
	//delete/ubah status jadi nonaktif => DTL ROLE WO
		$datarwd = array(  
					 'role_dtl_wo_status' => 2,
					); 
		$execrwd = $con->update("laundry_role_dtl_wo",$datarwd,"role_wo_id = '$_GET[d]'");
	// end //delete/ubah status jadi nonaktif 

		$selsequ = $con->select("laundry_role_wo","*","role_wo_seq > '$_GET[e]' and role_wo_createdby = '$_GET[f]'");
		foreach ($selsequ as $sequ) {
				$seqdb = $sequ['role_wo_seq']-'1';
				$datasequ = array(
				'role_wo_seq' => $seqdb,
		...
		//select lot no pada role child
		$sellotchild = $con->select("laundry_role_child","role_child_id,role_child_status","role_wo_id = '".$_GET['d']."'");
		foreach ($sellotchild as $lotchild) {
			if($lotchild['role_child_status'] == '1'){
				//delete/ubah status jadi nonaktif
					$datacd = array(  
								 'role_child_status' => 2,
								); 
					$execcd = $con->update("laundry_role_child",$datacd,"role_wo_id = '$_GET[d]'");
				// end //delete/ubah status jadi nonaktif 
			}
			else if($lotchild['role_child_status'] == 0){
				$where = array('role_child_id' => $lotchild['role_child_id']);
				$con->delete("laundry_role_child",$where);

				//ubah sequence / menyesuaikan sequence
					$selchildseq = $con->select("laundry_role_child","role_child_id,role_wo_seq","role_wo_seq > '".$_GET[e]."'");
					foreach ($selchildseq as $cseq) {
						$newseq = $cseq['role_wo_seq']-1;
						$dataseq = array(  
						 'role_wo_seq' => $newseq,
...
// hapus edit proses ===============================================================================
	...
	//hapus detail process saat after save dan di edit cmt process
			...
			$selsequ = $con->select("laundry_role_dtl_wo","*","role_dtl_wo_seq > '$_GET[seq]' and role_wo_id = '$_GET[id]'");
			foreach ($selsequ as $sequ) {
			...
			$selsum = $con->select("laundry_role_dtl_wo","SUM(role_dtl_wo_time) as time","role_wo_id = '$_GET[id]'");
			foreach ($selsum as $sum) {}

				$datasum = array('role_wo_time' => $sum['time']);
				$execsum = $con->update("laundry_role_wo",$datasum,"role_wo_id = $_GET[id]");
			...
			//select lot no pada role child
			$sellotchild = $con->select("laundry_role_child","role_child_id,role_child_status","role_dtl_wo_id = '".$_POST['hpsdtl']."'");
			foreach ($sellotchild as $lotchild) {
				if($lotchild['role_child_status'] == 1){
					//delete/ubah status jadi nonaktif
					$datacd = array(  
					...
					$execcd = $con->update("laundry_role_child",$datacd,"role_dtl_wo_id = '".$_POST['hpsdtl']."'");
					...
					//ubah sequence / menyesuaikan sequence
					$selchildseq = $con->select("laundry_role_child","role_child_id,role_dtl_wo_seq","role_dtl_wo_seq > '".$_GET[seq]."'");
					foreach ($selchildseq as $cseq) {
						$newseq = $cseq['role_dtl_wo_seq']-1;
						$dataseq = array(  
						 'role_dtl_wo_seq' => $newseq,
						); 
						$execseq = $con->update("laundry_role_child",$dataseq,"role_child_id = '$cseq[role_child_id]'");
					}
					//end ubah sequence
...
// echo "15";
	$jmlappwo = $_POST['jmlappwo'];
		...
		//master role wo
		$selkeranjang = $con->select("laundry_wo_master_keranjang","*","wo_master_keranjang_createdby = '".$_SESSION['ID_LOGIN']."'");
			...
			$roleid = $woid[3];
				...
				if ($value == 1){
					...
					foreach ($selrolewo as $rolewo) {
						...
						$selwoaja = $con->select("laundry_role_wo","COUNT(role_wo_id) as jmlwoid","role_wo_master_id = '".$idrolewom."' and role_wo_name = '".$rolewo['role_grup_name']."' and master_type_process_id =  '".$rolewo['master_type_process_id']."' and role_wo_seq = '".$rolewo['role_grup_seq']."' and role_wo_createdby = '".$_SESSION['ID_LOGIN']."'");		
		...
		{	
			...
			$roleid = $woid[3];
				...
				if ($value == 1){
					...
					$execwot = $con->update("laundry_role_wo",$datawot,"role_wo_id = '".$roleid."'");
...
//pengecekan apakah ada perubahan role process atau tidak
	...
	//select lot no pada role child
	$sellotchild = $con->select("laundry_role_child A JOIN laundry_role_wo B ON A.role_wo_id=B.role_wo_id","lot_no,lot_id,lot_type,role_wo_name_seq","A.role_wo_master_id = '".$_POST['role_wo_m_id']."' GROUP BY lot_no,lot_id,lot_type,role_wo_name_seq");

	foreach ($sellotchild as $lotchild) {
		foreach($con->select("laundry_role_wo","max(role_wo_name_seq) as role_wo_name_seq","role_wo_master_id = '".$_POST['role_wo_m_id']."' and master_type_process_id = 2") as $maxrolenameseq){}
			
			include "updaterolechildrec.php";
		...
		// 	//echo "B";
		// 	include "updaterolechildlot.php";
		// }
		...
		// 	//echo "D";
		// 	include "insertrolechildlotM.php";
		// }
	}
//end pengecekan apakah ada perubahan role process atau tidak
...
// echo "18";
	...
	$prb = $con->selectcount("laundry_role_wo_rev","role_wo_rev_id","role_wo_master_id = '$_POST[modsequenceeditid]' and role_wo_rev_createdby = '$_SESSION[ID_LOGIN]' and role_wo_rev_status = 0");
	// ============================================
