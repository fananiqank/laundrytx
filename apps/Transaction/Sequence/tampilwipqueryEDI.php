			...
			echo "<script>
				...
				$wherewomas1 = "A.wo_no ILIKE '%' 
								".$denimno." 
							  	...
							  	".$tgls."
							  	AND substring(A.wo_no,7,2) <> 'WS'
								AND concat ( A.wo_no, '_', A.garment_colors) $notin ( SELECT concat (wo_no,'_',garment_colors) FROM laundry_wo_master_dtl_proc )
		...
		<table width="100%" style="font-size: 12px;">
			...
			<tr>
				<td width="5%" valign="top">
		    		<input type="checkbox" id="cmtno_<?php echo $no; ?>" name="cmtno[]" value="<?php echo $wm[wo_no].'|'.$wm[garment_colors].'|'.$wm[buyer_id].'|'.$wm[ex_fty_date].'|'.$_GET[statusseq].'|'.$seqcode.'|'.$wm[type_source].'|'.$wm[buyer_style_no].'|'.$color_wash; ?>"> 
				</td>
