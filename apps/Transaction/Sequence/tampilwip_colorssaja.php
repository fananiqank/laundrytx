...
if ($denim_no){
	...
	// 						WHERE
	// 							A.wo_no ILIKE'%' $denimno $buyerno $styleno $cmtno $colorno $tgls	
	// 							AND concat ( A.wo_no, '_', C.garment_colors) $notin ( SELECT concat (wo_no,'_',garment_colors) FROM laundry_wo_master_dtl_proc )
	// 						GROUP BY
	// 							A.wo_no,
	// 							C.garment_colors,
	// 							E.buyer_id 
	...
	if ($cek == 0){
    	...
    	$selwomas = $con->select("work_orders
							  ...
							  JOIN order_instructions E ON A.order_no = E.order_no
							  JOIN quotation_header F ON E.quote_no = F.quote_no ",
							  "COUNT(A.wo_no) as jmlwo,A.wo_no,C.garment_colors,E.buyer_id,SUM ( C.quantity ) AS qty",
							  "A.wo_no ILIKE'%' $denimno $buyerno $styleno $cmtno $colorno $tgls
							  ...
							  ORDER BY
								A.wo_no");
    	echo "select COUNT(A.wo_no) as jmlwo,A.wo_no,C.garment_colors,E.buyer_id,SUM ( C.quantity ) AS qty,B.ex_fty_date 
    	      from work_orders A 
    	           JOIN wo_bpo B ON A.wo_no = B.wo_no
				   JOIN wo_sb C ON A.wo_no = C.wo_no AND B.buyer_po_number = C.buyer_po_number
				   JOIN order_instructions E ON A.order_no = E.order_no
				   JOIN quotation_header F ON E.quote_no = F.quote_no 
			 where A.wo_no ILIKE'%' $denimno $buyerno $styleno $cmtno $colorno $tgls
					AND concat ( A.wo_no, '_', C.garment_colors) NOT IN ( SELECT concat ( wo_no, '_', garment_colors) FROM laundry_wo_master_keranjang ) 
...
?>
		<tr>
			<td width="5%">
	    		<input type="checkbox" id="cmtno_<?=$no?>" name="cmtno[]" value="<?php echo $wm[wo_no].'_'.$wm[garment_colors].'_'.$wm[qty].'_'.$wm[buyer_id].'_'.$wm[ex_fty_date].'_'.$_GET[statusseq]; ?>"> 
			</td>
