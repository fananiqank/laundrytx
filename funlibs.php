...
class Database extends PDO{
    ...
    */
	...
	public function insertUSDN($table,$rows=null,$parconflict=null,$err=null) 
	{
		$command = 'INSERT INTO '.$table;
		$row = null; 
		$value=null;
		...
		foreach ($parconflict as $key => $parnya) {
			$param .=",".$key;
		}
		$command .="(".substr($row,1).")";
		$command .="VALUES(".substr($value,1).")";
		...
		if($parconflict != null) 
		$command .=" ON CONFLICT (".$parconflict.") DO NOTHING";	
...
from mrekeningnasabah where SUBSTRING_INDEX(RNACCOUNT,'-',1)='1' order by SUBSTRING_INDEX(SUBSTRING_INDEX(RNACCOUNT,'-',-2),'-',1) desc limit 0,1;
    ...
    */
    ...
    static function sql_connect ( $sql_details )
    {
        try {
            $db = @new PDO(
                "pgsql:host=192.168.51.24;port=5433;dbname=eis",
                'postgres',
                'youC1000',
