...
foreach ($con->select("(select lot_no as lotno, 
                        CASE 
                        WHEN lot_type != 'W' and lot_type != 'S' and lot_type != 'T' and lot_type != 'M'
...
c.wo_no,c.garment_colors,c.type_source,shift_id,TO_CHAR( C.ex_fty_date, 'DD-MM-YYYY' ) AS ex_fty_date,C.buyer_id,a.lot_createdate,lot_type 
...
where lot_no = '$datano' 
        UNION select rec_no as lotno, rec_qty as qty, rec_status as status, rec_id as id,d.wo_master_dtl_proc_id,'1' as role_wo_name_seq,
            f.wo_no,f.garment_colors,f.type_source,shift_id,TO_CHAR( f.ex_fty_date, 'DD-MM-YYYY' ) AS ex_fty_date,f.buyer_id,d.rec_createdate,rec_type::TEXT 
...
"lotno,qty,status,id,wo_master_dtl_proc_id,wo_no,garment_colors,shift_id,ex_fty_date,buyer_id,lot_createdate,role_wo_name_seq,CASE WHEN type_source = 3 THEN 'QRCODE' ELSE 'MANUAL' END as type_source,lot_type") as $cmt) {}
...
if ($cmt['lot_type'] == 'Q') {
    $title = 'QA Sample';
    $ref = '';
} else if ($cmt['lot_type'] == 'R') {
    $title = 'Reject Lot';
    $ref = '';
} else if ($cmt['lot_type'] == 'N') {
    $title = 'Normal Prod';
    $ref = '';
} else if ($cmt['lot_type'] == 'C') {
    $title = 'Combine Lot';
    $ref = '<tr>
              <td width="30%">Ref. Lot No</td>
              <td width="70%">: ' . $datano . '</td>
          </tr>';
} else if ($cmt['lot_type'] == 'B') {
    $title = 'Split Lot';
    $ref = '<tr>
              <td width="30%">Ref. Lot No</td>
              <td width="70%">: ' . $datano . '</td>
          </tr>';
} else if ($cmt['lot_type'] == 'P') {
    $title = 'Pre Bulk Lot';
    $ref = '';
} else if ($cmt['lot_type'] == 'F') {
    $title = 'First Bulk Lot';
    $ref = '';
} else if ($cmt['lot_type'] == 'W') {
    $title = 'Rework QA Lot';
    $ref = '';
} else if ($cmt['lot_type'] == 'M') {
    $title = 'Rework Lot';
    $ref = '';
} else if ($cmt['lot_type'] == 'S') {
    $title = 'Scrap Lot';
    $ref = '';
} else if ($cmt['lot_type'] == 'T') {
    $title = 'Standart Lot';
    $ref = '';
}
else {
