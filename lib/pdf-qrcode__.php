...
c.wo_no,c.garment_colors,c.type_source,shift_id,TO_CHAR( C.ex_fty_date, 'DD-MM-YYYY' ) AS ex_fty_date,C.buyer_id,TO_CHAR( a.lot_createdate, 'DD-MM-YYYY' ) AS lot_createdate 
from laundry_lot_number a 
JOIN laundry_wo_master_dtl_proc c ON a.wo_master_dtl_proc_id=c.wo_master_dtl_proc_id 
where lot_no = '$datano' 
        UNION select rec_no as lotno, CASE WHEN COALESCE(rec_qty_good_upd,0) != 0 THEN rec_qty_good_upd 
                                 ELSE rec_qty END as qty, rec_status as status, rec_id as id,d.wo_master_dtl_proc_id,'1' as role_wo_name_seq,
            f.wo_no,f.garment_colors,f.type_source,shift_id,TO_CHAR( f.ex_fty_date, 'DD-MM-YYYY' ) AS ex_fty_date,f.buyer_id,TO_CHAR( d.rec_createdate, 'DD-MM-YYYY' ) AS lot_createdate 
...
$pdf->judul($title, $datano, $cmt['wo_no'], $cmt['garment_colors'], $cmt['buyer_id'].' | efd: '.$cmt['ex_fty_date'], $cmt['lot_createdate'], 'Qty: '.$cmt['qty'], $cmt['shift_id'].' | '.$cmt['role_wo_name_seq'],$cmt['type_source']);
$pdf->Output();
